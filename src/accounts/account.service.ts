...
async function getRefreshToken(token: any) {
    ...
    if (!refreshToken || !refreshToken.isActive) throw 'Refresh token not found';
    return refreshToken;
}
...
async function generateJwtToken(account: any) {
    return jwt.sign(
...
function basicDetails(account: any) {
    const { passwordHash, ...accountWithoutHash } = account.toJSON();
...
async function sendVerificationEmail(account: any, origin: any) {
    ...
    if (origin) {
        ...
        message = `
            ...
            <p><b>${account.verificationToken}</b></p>
        `;
    }
    await sendEmail({
        to: account.email,
...
async function sendAlreadyRegisteredEmail(email: string, origin: any) {
    let message = '';
    if (origin) {
        message = `<p>If you don't know your password please visit the <a href="${origin}/account/forgot-password">Reset your password</a> page.</p>`;
    } else {
...
async function sendPasswordResetEmail(account: any, origin: any) {
    ...
    if (origin) {
        ...
        message = `<p>Please use the below token to reset your password <code>/account/reset-password</code> api route.</p>
